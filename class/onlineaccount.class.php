...
class onlineaccount extends SeedObject
{
	/**
	 * Draft status
	 */
	...
	public $table_element = 'onlineaccount';

	public $element = 'onlineaccount';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'onlineaccount', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showonlineaccount") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/onlineaccount/card.php', 1).'?id='.$this->id. $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='onlineaccountStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='onlineaccountStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='onlineaccountStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='onlineaccountStatusAccepted'; $shortkeytrans='Accepted'; }
...
/*
class onlineaccountDet extends TObjetStd
{
	public $table_element = 'onlineaccountdet';

	public $element = 'onlineaccountdet';
